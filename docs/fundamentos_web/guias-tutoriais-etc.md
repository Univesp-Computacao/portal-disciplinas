# Guias, Tutoriais e Referências para os fundamentos da Web

:pushpin: Lista composta de materiais diretamente relacionados aos Fundamentos da Web.

:construction: Ajude-nos a construir essa lista: dê suas sugestões!
### Referências
- [Consórcio World Wide Web (pt-BR)](https://www.w3c.br)
- [IETF - Internet Engineering Task Force (en)](https://www.ietf.org)
- [W3 Schools  (pt-BR)](https://www.w3schools.com/html/default.asp)
- [HTTP - mdn web docs (pt-BR)](https://developer.mozilla.org/pt-BR/docs/Web/HTTP)
- [Vocabulário para microdata (en)](https://schema.org/docs/gs.html)

### Tutoriais
- [Learn to Code HTML & CSS (en)](https://learn.shayhowe.com)
- [CSS Reference (en)](https://cssreference.io)
- [CSS Basics (en)](https://www.cssbasics.com)
- [Semantic HTML (en)](https://internetingishard.netlify.app/html-and-css/semantic-html/index.html) 

### Diversos
- [Os personagens do Simpsons desenhados com CSS puro](https://pattle.github.io/simpsons-in-css)
- [Unsplash - Imagens de uso livre](https://unsplash.com/)
- [Figma - Software de design](https://www.figma.com)
- [Font Awesome - ícones de uso gratuito](https://fontawesome.com/)
- [Fonts Google - fontes de uso gratuito](https://fonts.google.com/)
- [Gradientes prontos](https://webgradients.com/)
- [Outros gradientes](https://cssgradient.io/gradient-backgrounds/)
- [Exemplos de layouts para se inspirar](https://www.webdesign-inspiration.com/)
- [Web Skills - habilidades Web ](https://andreasbm.github.io/web-skills/)
- [Origamid - Flex Box](https://origamid.com/projetos/flexbox-guia-completo/)
- [CSS Tricks](https://css-tricks.com)